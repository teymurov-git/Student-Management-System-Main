...
2. Vercel repo-nu "multiple services" kimi göstərsə, `vercel.json` içindəki
   `experimentalServices` konfiqurasiyasını istifadə edin: yalnız `app`
   servisi deploy olunmalıdır (`api/index.py`, route `/`). Ayrı `backend`
   və ya `frontend` servisi seçməyin.
3. Framework kimi **Other** seçin. Əgər import ekranında **Services** seçimi
   məcburidirsə, `app` servisinin qaldığını və route-un `/` olduğunu yoxlayın.
